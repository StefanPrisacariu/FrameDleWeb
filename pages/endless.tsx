...
function Endless() {
    ...
    const [todaysWf, setTodaysWf] = useState<Warframe>(
        initialWarframes[Math.floor(Math.random() * (initialWarframes.length + 1))] as Warframe
    );
    ...
    const handleChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSearchText(value);
        setVisible(true);
        setFilteredWarframes(initialWarframes.filter(wf => wf.name.toLowerCase().includes(value.toLowerCase())));
    }, []);
    ...
    const newWarframe = useCallback(() => {
        setIsGuessed(false);
        setGuesses([]);
        setTodaysWf(initialWarframes[Math.floor(Math.random() * (initialWarframes.length + 1))] as Warframe);
    }, []);
                ...
                <div className={Container.fd_container_0}>
                    ...
                    {todaysWf && (
                        <>
                            ...
                            <h2 className={clsx(Group.fd_group_0_label, Group.fd_group_pb_10)}>
                                <span className={Text.fd_text_0}>Endless</span>
                                <button onClick={() => newWarframe()} className={Button.fd_button_5}>
                                    New Warframe
                                </button>
                            </h2>
                            <div className={Container.fd_container_2}>
                                <div className={Input.fd_input_0}>
                                    {!isGuessed && (
                                        <div className={Input.fd_input_0_wrapper}>
                                            ...
                                            >
                                                <div className={Input.fd_input_0_wrapper_button_content}>
                                                    {visible ? (
                                                        <DropdownX className={Input.fd_input_0_wrapper_button_symbol} />
                                                    ) : (
                                                        <DropdownArrow
                                                            className={Input.fd_input_0_wrapper_button_symbol}
                                                        />
                                                    )}
                                                </div>
                                            </button>
                                            ...
                                            {filteredWarframes.map(item => (
                                                ...
                                                >
                                                    <Image
                                                        ...
                                                        height={40}
                                                        src={item.image}
                                                        className={Dropdown.fd_dropdown_0_item_image}
                                                        alt={item.name}
                                                    />
                                                    <span className={Dropdown.fd_dropdown_0_item_text}>
                                                        {item.name}
                                                    </span>
                                                </button>
                                            ))}
                                ...
                                <h4 className={Text.fd_text_1}>
                                    Attempts
                                    {445 >= (width || 0) && (
                                        <span className={Text.fd_text_1_info}>{`<- Scroll for more info ->`}</span>
                                    )}
                                </h4>
                                        ...
                                        {todaysWf && guesses.length > 0
                                            ? [...guesses]
                                                  .map((item, index) => (
                                                      ...
                                                      >
                                                          <GuessRow warframeGuess={item} todayWarframe={todaysWf} />
                                                      </motion.div>
                                                  ))
                                                  .reverse()
