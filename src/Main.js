...
import { getDailyStreak, getDailyStreakTime } from './helpers/storeReadStreak.js';
import { storeGuesses, getGuesses } from './helpers/storeReadGuesses.js';

import { isLowerThanToday320AM, isLowerThanToday320AMForGuesses } from './helpers/isLowerThan320.js';
...
function Main() {
    ...
    const fetchGuesses = async () => {
        ...
        if (todaysWf && storedGuesses.length > 0 && storedGuesses[storedGuesses.length - 1]?.name === todaysWf.name) {
            setIsGuessed(true);
        } else {
            setIsGuessed(false);
        }
    ...
    const warframeSelected = useCallback(
        async e => {
            ...
            if (todaysWf !== null) {
                ...
                await storeGuesses([...guesses, e]).catch(error => console.error(error));
    ...
    const updateDailyStreak = async () => {
        try {
            await localStorage.setItem('FD_DAILY_STREAK', JSON.stringify(dailyStreak + 1));
