import { polarity, elements } from '@/app/helpers/imageExports';
...
export const GuessRow = ({ warframeGuess, todayWarframe }: Guess) => {
    ...
    const handleIcon = (state: string) => {
        if ('Disabled' !== colorblind) {
            switch (state) {
                case 'correct':
                    return <Correct className={clsx(Icon.fd_icon_0, Icon.fd_icon_p_3)} width={55} height={55} />;
                case 'incorrect':
                    return <Incorrect className={clsx(Icon.fd_icon_0, Icon.fd_icon_p_5)} width={50} height={50} />;
                case 'partial':
                    return <Partial className={clsx(Icon.fd_icon_0, Icon.fd_icon_p_3)} width={60} height={60} />;
                default:
                    return null;
            }
        }
            ...
            <div className={GR.fd_gr_0}>
                ...
                <div
                    className={clsx(GR.fd_gr_0_box, {
                        [`${handleColorblind('correct')}`]: warframeGuess.gender === todayWarframe.gender,
                        [`${handleColorblind('incorrect')}`]: warframeGuess.gender !== todayWarframe.gender,
                    })}
                >
                ...
                <div
                    className={clsx(GR.fd_gr_0_box, {
                        [`${handleColorblind('correct')}`]: warframeGuess.primeUmbra === todayWarframe.primeUmbra,
                        [`${handleColorblind('incorrect')}`]: warframeGuess.primeUmbra !== todayWarframe.primeUmbra,
                    })}
                >
                    {warframeGuess.primeUmbra === todayWarframe.primeUmbra
                        ? handleIcon('correct')
                        : handleIcon('incorrect')}
                    <p className={GR.fd_gr_0_text}>{warframeGuess.primeUmbra}</p>
                </div>
                <div
                    className={clsx(GR.fd_gr_0_box, {
                ...
                >
                    ...
                    <p className={GR.fd_gr_0_text}>{warframeGuess.progenitorElement}</p>
                </div>
                <div
                    className={clsx(GR.fd_gr_0_box, {
                        [`${handleColorblind('correct')}`]: warframeGuess.releaseYear === todayWarframe.releaseYear,
                        [`${handleColorblind('incorrect')}`]: warframeGuess.releaseYear !== todayWarframe.releaseYear,
                    })}
                >
                    {warframeGuess.releaseYear !== todayWarframe.releaseYear &&
                    warframeGuess.releaseYear < todayWarframe.releaseYear ? (
                        <Arrow className={clsx(Icon.fd_icon_0, Icon.fd_icon_p_3)} width={50} height={50} />
                    ) : (
                        warframeGuess.releaseYear !== todayWarframe.releaseYear && (
                            <Arrow
                                className={clsx(Icon.fd_icon_0, Icon.fd_icon_p_3, Icon.fd_icon_reverse)}
                                width={50}
                                height={50}
                            />
                        )
                    ...
                    <p className={GR.fd_gr_0_text}>{warframeGuess.releaseYear}</p>
                </div>
            </div>
        </>
    );
